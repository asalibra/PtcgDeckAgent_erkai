## 从弃牌堆拿回支援者卡
## Beckon - Audino: put a Supporter card from discard into hand
class_name AttackRetrieveSupporterFromDiscard
extends BaseEffect
...
func execute_attack(
	...
	# Find supporter cards in discard
	var supporter_cards: Array[CardInstance] = []
	for card: CardInstance in player.discard_pile:
		if card.card_data != null and card.card_data.card_type == "Supporter":
			supporter_cards.append(card)

	if supporter_cards.is_empty():
		return

	# Move the first supporter card to hand
	var chosen: CardInstance = supporter_cards[0]
	player.discard_pile.erase(chosen)
	player.hand.append(chosen)

	if state.battle_log != null:
		state.battle_log.log_action("effect", "从弃牌堆拿回支援者卡: %s" % chosen.card_data.name)


func get_description() -> String:
	return "从自己的弃牌堆选择1张支援者卡，加入手牌。"

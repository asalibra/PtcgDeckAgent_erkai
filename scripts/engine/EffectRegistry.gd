...
## ==================== 英文特性名 → 效果实例映射 ====================
## 当中文名匹配失败时自动回退到英文名匹配
	static func _get_ability_effect_en(_ability_name: String) -> BaseEffect:
	# DISABLED - contains references to non-existent effect classes
	return null
...
## 当中文名匹配失败时自动回退到英文名匹配
static func _get_attack_effects_en(_processor: EffectProcessor, _attack_name: String) -> Array:
	# DISABLED - contains references to non-existent effect classes
	# TODO: re-enable when all effect scripts are created
	return []
